...
{
    /// <summary>
    /// Tile 类及其派生类 (Floor, Wall, Roof, Door) 的自定义属性绘制器。
    /// 在折叠标题栏右侧添加一键 "Reset" 按钮，解决空列表添加项时默认值被 Unity 清零的缺陷。
    /// </summary>
    [CustomPropertyDrawer(typeof(Tile), true)]
    public class TileDrawer : PropertyDrawer
    {
        ...
        /// </summary>
        private void ResetTileProperties(SerializedProperty property)
        {
            try
            {
                // 1. 寻找 property 映射的底层运行时真实对象引用
                object targetObj = GetTargetObjectWithPropertyPaths(property);
                if (targetObj is Tile tile)
                {
                    // 2. 注册撤销记录，使用 targetObject
                    Undo.RecordObject(property.serializedObject.targetObject, "Reset Tile Properties");
            ...
            {
                UnityEngine.Debug.LogError($"重置 Tile 属性失败: {ex.Message}");
            }
        }
